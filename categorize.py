...
path = r'H:\Work\RE_Control\src\main\game-bridge\VotingManager.ts'
with open(path, 'r', encoding='utf-8') as f:
    content = f.read()

categorizedPool = '''    private effectPool = [
        // Positive
...
content = re.sub(r'private effectPool = \[[\s\S]*?\];', categorizedPool, content)

old_logic = '''        const shuffled = [...this.effectPool].sort(() => 0.5 - Math.random());
...
new_logic = '''        const posPool = this.effectPool.filter(e => e.category === 'positive');
